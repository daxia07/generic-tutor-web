...
function SessionFlow() {
  ...
  const handleStartNextConcept = useCallback(() => {
    const conceptId = result?.nextConceptId || sessionMeta?.nextConceptId;
    if (conceptId) {
      router.push(`/session?conceptId=${encodeURIComponent(conceptId)}&mode=learn`);
    } else {
      router.push("/");
    }
  }, [result, sessionMeta, router]);
  ...
  const nextTitle = result?.nextConceptTitle || sessionMeta?.nextConceptTitle;
          ...
          >
            {result ? (
              <SessionComplete
                ...
                nextSessionTitle={nextTitle ?? undefined}
                onStartNext={nextTitle ? handleStartNextConcept : undefined}
              />
            ) : (
